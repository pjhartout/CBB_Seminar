...
\usetheme{metropolis}
\usepackage{appendixnumberbeamer, lmodern, bookmark, kbordermatrix}
\usepackage{booktabs}
...
\subtitle{by Gibson \emph{et al} -- Computational Biology and Bioinformatics Seminar}
\date{\today}
...
\begin{frame}[fragile]{Motivation}
    ...
    The organisation of cells into epithelial sheets is an essential feature of animal Fdesign.
    \end{aquote}
...
\begin{frame}[fragile]{Geometry of \emph{growing} epithelial cells}    
    ...
    \begin{columns}[onlytextwidth]
        ...
        Time-lapse movies collected revealed the following findings:
          \begin{itemize}
            \onslide<2->\item Initially polygonal prophase cells rounded up and divided
            \onslide<3->\item Cell-neighbour relationships were stably maintained throughout the cell cycle
            \onslide<4->\item The topology of the divided cells is invariant over time.
          \end{itemize}
...
\begin{frame}[fragile]{A discrete Markov model to model stochastic cell proliferation}
  \begin{columns}
    \column{\textwidth}
    \begin{table}
      \centering
      ...
      \begin{tabular}{ll}
        ...
        $p_s$ & the relative frequency of $s$-sided cells in the population \\
        $\mathbf{p}^{(t)}$ & the infinite row vector $\mathbf{p}^{(t)}=[p_4, p_5, p6,\ldots ]$ state of the population at generation $t$.\\
        $\mathbf{p}^{(t+1)}=\mathbf{p}^{(t)}PS$ & the state dynamics, where $P$ and $S$ are the probabilistic transition matrices.\\
...
\begin{frame}[fragile]{Derivation of Markov state dynamics - transition matrix $S$}
  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{itemize}
      ...
      \onslide<5->\item Thus, $S_{ij}=1$ if $j=i+!$ and 0 otherwise.
    \end{itemize}
    \column{0.5\textwidth} 
      \renewcommand{\kbldelim}{(}% Left delimiter
      \renewcommand{\kbrdelim}{)}% Right delimiter
...
\begin{frame}[fragile]{Take home messages} 
  ...
  \begin{enumerate}
    ...
    \onslide<2->\item \textbf{explains the hexagonal topology} of growing epithelia
    \onslide<3->\item predicts the \textbf{overall distribution} of polygonal cell types
    \onslide<4->\item shows an \textbf{emergent mechanism} by which epithelia accomodate \textbf{rapid proliferation} while maintaining \textbf{structural integrity}. 
    \onslide<5->\item Provides a \textbf{framework} for investigating other models for \textbf{cell division} such as \textbf{cleavage plane} choices or \textbf{aberrant cell division}.
  \end{enumerate}
\end{frame}
